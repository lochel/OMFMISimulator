...
static int OMFitLua_omsfit_initialize(lua_State *L)
{
  ...
  // create buffers
  double* time = malloc(nTime*sizeof(double));
  char const* * inputvars = malloc(nInputvars*sizeof(char));
  char const* * measurementvars = malloc(nMeasurementvars*sizeof(char));
...
static int OMFitLua_omsfit_addMeasurement(lua_State *L)
{
  ...
  double* values = malloc(nValues*sizeof(double));
  int i;
  for (i=0; i < nValues; ++i) {
